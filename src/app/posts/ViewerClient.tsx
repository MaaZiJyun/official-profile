    ...
    async function ensureAndRender() {
      try {
        ...
        if (!document.querySelector("style[data-latexjs-theme]")) {
          ...
          theme.textContent = `
            /* Scoped LaTeX rendering styles — IEEEtran-inspired */
            .latex-rendered { font-family: var(--site-font, Times New Roman), serif; color: inherit; line-height: 1.3; }
            .latex-rendered h1 { font-variant: small-caps; font-weight: 600; text-align: center; margin-top: 1.5ex; margin-bottom: 0.7ex; letter-spacing: 0.5px; }
            .latex-rendered h2 { font-style: italic; font-weight: 500; margin-top: 1.2ex; margin-bottom: 0.5ex; }
            .latex-rendered h3 { font-style: italic; font-weight: 500; margin-top: 0.5ex; margin-bottom: 0.3ex; }
            ...
            .latex-rendered table { border-collapse: collapse; width: 50%; margin: 0.75rem auto; border: none; font-size: 0.8rem; }
            .latex-rendered th, .latex-rendered td { border: none; padding: 0.2rem 0.5rem; }
            .latex-rendered li { font-size: 1rem; display: flex; align-items: baseline; gap: 0.5rem; }  
            .latex-rendered li > p { margin: 0; }
            .latex-rendered code { background: #f3f4f6; padding: 0.12rem 0.3rem; border-radius: 4px; font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, 'Roboto Mono', 'Helvetica Neue', monospace; font-size: 0.833rem; }
            ...
            .latex-rendered .title { font-size: 1.5rem; font-weight: 700; text-align: center; margin-bottom: 1rem; }
        ...
        // Strip any <link> tags generated by latex.js for local CSS (e.g. article.css)
        outHtml = outHtml.replace(/<link[^>]*href=["'][^"']*\.css["'][^>]*\/?>/gi, "");
        ...
        processedHtml = processedHtml.replace(
          ...
          (m: string) => {
            ...
            if (type === "cap") {
              ...
              return `<div style="font-weight:600; font-size:0.75rem; margin-top:0.5rem; text-align:center;">${esc}</div>`;
            }
            if (type === "table") {
              try {
        ...
        processedHtml = processedHtml.replace(
          ...
          (_m: string, inner: string, encCap: string) => {
            ...
            return `<div style="text-align:center; margin:1rem 0;">${inner}${
              cap
                ? `<div style=\"font-weight:600; font-size:0.75rem; margin-top:0.5rem;\">${cap}</div>`
                : ""
            }</div>`;
          }
        );
